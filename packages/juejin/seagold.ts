...
export class SeaGold {
  ...
  getNodePosition(map: any[], node: any) {
  ...
  getRoutePath(map: any, startNode: any, endNode: any) {
  ...
  createMapNode(x: number, y: number, secret: any) {
    const rule = this.getNodeRule(secret) as NodeRule;
    return {
      code: rule.code,
  ...
  getBounty(secret: any, key: string) {
...
export async function runSeaSold(cookie: string) {
